      ...
      <div>
        <div>
          <div className="flex items-center justify-between gap-2 w-full overflow-x-hidden">
            ...
            {/* Prev Button */}
            <div className="shrink-0 grow-0 flex items-center">
              <button
            ...
            {/* Card Grid */}
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 w-full overflow-x-hidden">
              {currentImages.map((image, index) => (
                <motion.div
                  key={index}
                  className="flex flex-col items-center rounded-tl-3xl rounded-br-3xl bg-[#f3cdba5d] shadow-md w-full max-w-full box-border"
                ...
                >
                  ...
                  >
                    <img
                      className="rounded-tl-3xl bg-[#FEB38D] p-1 w-full max-w-full"
                      src={image.src}
                      alt={image.title}
                    />
                  </LazyLoad>
...
<AnimatePresence>
  {isModalOpen && selectedImage && (
    <motion.div
      className="fixed inset-0 bg-black bg-opacity-60 flex justify-center mt-4 md:mt-16 items-center z-50 overflow-auto p-4"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      onClick={closeModal}
    >
      <motion.div
        className="bg-white rounded-lg shadow-lg w-[90%] max-w-2xl p-4 md:p-8 max-h-[90vh] overflow-y-auto"
      ...
      >
        ...
        <div className="flex flex-row items-center justify-center gap-4 mb-4">
          ...
          >
            <motion.img
              ...
              alt="Main"
              className="w-full max-w-xs sm:max-w-sm md:max-w-md mx-auto max-h-[200px] sm:max-h-[250px] md:max-h-[300px] rounded border object-cover"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <textarea
            rows={8}
            className="md:col-span-2 py-2 text-sm text-justify resize-none w-full"
            readOnly
